...
The format checkers for the subtasks A, B and C are located in the format_checker module in the each subtask directory.
...
The scorer for the subtasks is located in the scorer module in the each subtask directory.
The scorer will report official evaluation metric and other metrics of a prediction file.

### Subtask A:
The **official evaluation metric** for the task is **accuracy**. However, the scorer also reports macro-F1 and micro-F1. 
...
The average results for multilingual setup across three runs for XLM-R is 0.72;
